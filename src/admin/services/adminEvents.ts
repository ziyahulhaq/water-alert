...
export async function listEvents(opts?: { search?: string; dateFrom?: string; dateTo?: string; limit?: number }): Promise<WaterEvent[]> {
  let query = supabase
    .from('water_events')
    .select('id, device_id, detected_at, water_level, devices(mac_hash, model_id)')
  ...
  return (data ?? []).map((e: any) => ({
    ...
    device_mac: e.devices?.model_id ?? e.devices?.mac_hash?.slice(0, 12) ?? e.device_id.slice(0, 8),
  }));
}
