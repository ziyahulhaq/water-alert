...
import {
  Users, Cpu, Layers, Wifi, WifiOff, Activity,
  ...
  const fetchAll = async () => {
    ...
    try {
      const [statsData, usersData, devicesData, eventsData, logsData] = await Promise.all([
        ...
        supabase.from('devices').select('id,mac_id,status,last_seen').order('last_seen', { ascending: false, nullsFirst: false }).limit(5).then(r => r.data ?? []),
    ...
    <div className="space-y-8">
      ...
      {loading && !stats ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="Total Models"       value={stats.totalModels}       icon={Layers}  color="cyan"    />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900/50 border border-slate-800/60 rounded-2xl overflow-hidden">
          ...
          <Table headers={['MAC ID', 'Status', 'Last Seen']}>
            {devices.map(d => (
              <tr key={d.id} className="hover:bg-slate-800/20 transition-colors">
                <Td><span className="font-mono text-xs">{d.mac_id}</span></Td>
