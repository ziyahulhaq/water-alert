          ...
          <div className={`glass-card rounded-2xl overflow-hidden transition-all ${
            ...
            {profile?.chat_id ? (
              ...
              <div className="p-6 flex items-start gap-4">
                <div className="w-11 h-11 rounded-xl flex items-center justify-center shrink-0 bg-emerald-500/10 border border-emerald-500/20">
                  <CheckCircle2 className="w-6 h-6 text-emerald-400" />
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold uppercase tracking-wider text-emerald-400 mb-0.5">Telegram Alerts</p>
                  <p className="text-white font-bold">Connected ✓</p>
                  <p className="text-gray-400 text-xs mt-1 leading-relaxed">
                    Water alerts are being sent to your Telegram. Chat ID: <code className="bg-slate-900 px-1.5 py-0.5 rounded text-[10px] text-emerald-300 font-mono">{profile.chat_id}</code>
                  </p>
                </div>
              </div>
            ) : (
              ...
              <div>
                ...
                <div className="px-6 pt-6 pb-5 bg-gradient-to-r from-blue-950/60 via-slate-900/40 to-slate-900/40 border-b border-slate-800/60">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center bg-blue-500/15 border border-blue-500/25 shrink-0">
                      <Send className="w-5 h-5 text-blue-400" />
                    </div>
                    <div>
                      <p className="text-[10px] font-semibold uppercase tracking-wider text-blue-400">Step 2 — Telegram Alerts</p>
                      <p className="text-white font-bold text-sm">Connect your Telegram account</p>
                    </div>
                    {/* Pulse indicator */}
                  ...
                  <p className="text-gray-400 text-xs leading-relaxed">
                    Receive instant push notifications on Telegram the moment water supply starts or stops.
                  </p>
                </div>

                {/* How-it-works steps */}
                <div className="px-6 py-4 border-b border-slate-800/40">
                  <p className="text-[10px] font-semibold uppercase tracking-widest text-gray-500 mb-3">How it works</p>
                  <ol className="space-y-2">
                    {[
                      { n: '1', text: 'Click the button below to open Telegram' },
                      { n: '2', text: 'Press START in the bot chat — your account links automatically' },
                      { n: '3', text: 'Come back here — this card will show ✓ Connected' },
                    ].map((step) => (
                      <li key={step.n} className="flex items-start gap-2.5 text-xs text-gray-400">
                        <span className="shrink-0 w-5 h-5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 flex items-center justify-center text-[9px] font-bold">{step.n}</span>
                ...
                <div className="px-6 py-5 space-y-3">
                  <a
                    ...
                    className="flex items-center justify-center gap-2.5 w-full bg-blue-600 hover:bg-blue-500 active:scale-95 text-white font-bold px-4 py-3 rounded-xl transition-all shadow-lg shadow-blue-600/30 hover:shadow-blue-500/40 text-sm"
                  >
                    <Send className="w-4 h-4" />
                    <span>Open in Telegram → Start Bot</span>
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-full bg-slate-800/60 hover:bg-slate-800 border border-slate-700/50 hover:border-blue-500/30 text-gray-300 hover:text-white font-medium px-4 py-2.5 rounded-xl transition-all text-xs"
                  >
                    {linkCopied ? (
                      <><Check className="w-3.5 h-3.5 text-emerald-400" /><span className="text-emerald-400">Link copied!</span></>
                    ) : (
                      <><Copy className="w-3.5 h-3.5" /><span>Copy bot link (open on phone)</span></>
                    )}
                  </button>
                  ...
                  {profile?.link_token && (
                    <p className="text-center text-[10px] text-gray-600">
                      Token: <code className="font-mono text-gray-500">{profile.link_token.slice(0, 8)}…</code>
                    </p>
                  )}
                </div>
              </div>
