...
create policy "Users can view own device water events" on public.water_events
  for select using (
    exists (
      select 1 from public.devices
      where public.devices.id = public.water_events.device_id
      and public.devices.user_id = auth.uid()
    )
  );
create policy "Users can insert own device water events" on public.water_events
